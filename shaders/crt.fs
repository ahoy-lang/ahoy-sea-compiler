...
uniform float time;
uniform vec2 resolution = vec2(1280.0, 720.0);  // Default resolution

// CRT effect parameters with defaults
uniform float scanlineOpacity = 0.2;
uniform float scanlineWidth = 0.1;
uniform float grillOpacity = 0.0;
uniform float noiseOpacity = 0.01;
uniform float noiseSpeed = 0.1;
uniform float staticNoiseIntensity = 0.1;
uniform float aberration = 0.003;
uniform float brightness = 1.2;
uniform float warpAmount = 0.1;
